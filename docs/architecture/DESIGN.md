...
**Current live product:** `fairdrop_dutch_v4.aleo` (was `fairdrop_v4.aleo`) — a Dutch descending-price auction with uniform clearing price, dual private/public bid paths, and full `token_registry.aleo` + `credits.aleo` CPI integration. Working on Aleo Testnet Beta.
...
```
fairdrop_dutch_v4.aleo      ← live. Dutch descending-price auction.
...
```
fairdrop_dutch_v4.aleo      PROGRAM_SALT = 1field
...
**Record ownership:** records belong to their defining program. `fairdrop_dutch_v4.aleo/Bid` cannot be consumed by `fairdrop_sealed_v3.aleo`. Each auction type defines its own record types. Utility contracts define their own records (e.g. `fairdrop_proof_v3.aleo/ParticipationReceipt`).
...
### 4a. Dutch Auction (`fairdrop_dutch_v4.aleo`)
...
**Both bid paths (M3 fix):** like `fairdrop_dutch_v4.aleo`, the raise supports:
...
`fairdrop_v4.aleo` on Aleo Testnet Beta. Renamed to `fairdrop_dutch_v4.aleo` in the new architecture.
...
PHASE 1b  fairdrop_dutch_v4.aleo — NEW deployment (not an update to live v4)
...
CREATE TABLE auctions (
    ...
    program_id       TEXT NOT NULL,      -- "fairdrop_dutch_v4.aleo" etc.
...
```
GET  /auctions
     ?program=fairdrop_dutch_v4.aleo   (filter by type)
