...
import { Card, CardContent } from '@fairdrop/ui';
        ...
        {!closeable && !slashable && !hasReferral && (
          <p className="text-sm text-muted-foreground">
            {auction.status === AuctionStatus.Cleared || auction.status === AuctionStatus.Voided
              ? <>This auction is closed. Check <Link to={routes.earnings} className="underline">Earnings</Link> for referral commissions.</>
        ...
        {slashable && (
          <div className="rounded-md border border-border p-3 space-y-0.5">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              Available from the{' '}
              <Link to={routes.earnings} className="underline">Earnings</Link> page.
            </p>
          </div>
        )}
