...
The caller reads current params off-chain and passes them as `ConfigSnapshot` to the auction transition. Finalize calls `fairdrop_config_v1.aleo/get_config` and asserts the snapshot matches — ensuring no stale values are used (D11 pattern).
